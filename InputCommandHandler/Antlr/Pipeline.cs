...
namespace InputCommandHandler.Antlrr
{
    public class Pipeline : IAntlrErrorListener<IToken>
    {
        ...
        public void Transform(IPlayerService playerService, ISessionService sessionService, IAgentService agentService)
        {
            if (_ast == null)
            {
                return;
            }
            new Evaluator(playerService, sessionService, agentService).Apply(_ast);
        }
    }
}